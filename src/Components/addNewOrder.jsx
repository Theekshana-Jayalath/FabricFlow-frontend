...
function AddNewOrder() {
  ...
  const generatePDF = () => {
    try {
      ...
      doc.text(`Order ID: ${formData.orderId || 'Not Generated'}`, 20, 62);
      doc.text(`Order Date: ${formatDateForDisplay(formData.orderDate)}`, 20, 70);
      doc.text(`Delivery Date: ${formatDateForDisplay(formData.deliveryDate)}`, 20, 78);
      doc.text(`Order Status: ${formData.orderStatus}`, 20, 86);
      doc.text(`Payment Status: ${formData.paymentStatus}`, 20, 94);
      ...
      doc.text(`Name: ${formData.customer.name || 'Not provided'}`, 20, 122);
      doc.text(`Email: ${formData.customer.email || 'Not provided'}`, 20, 130);
      doc.text(`Phone: ${formData.customer.phone || 'Not provided'}`, 20, 138);
      ...
      // Summary content with fabric information
      doc.setFillColor(248, 249, 250);
      doc.rect(130, finalY, 60, 35, 'F');
      doc.setLineWidth(0.5);
      doc.rect(130, finalY, 60, 35);

      doc.setFontSize(12);
      doc.setFont("helvetica", "bold");
      doc.text("Order Summary", 135, finalY + 8);
      ...
      doc.text(`Total Items: ${totalItems}`, 135, finalY + 15);
      doc.setTextColor(34, 139, 34);
      doc.text(`Total Fabric: ${totalFabric.toFixed(1)}m`, 135, finalY + 22);
      doc.setFontSize(12);
      doc.setFont("helvetica", "bold");
      doc.setTextColor(220, 38, 127);
      doc.text(`Total Amount: Rs ${calculateTotal().toLocaleString()}`, 135, finalY + 30);
      ...
      doc.text("Thank you for choosing FabricFlow!", 20, footerY);
      doc.text("For any queries, contact us at:", 20, footerY + 8);
      doc.text("Email: support@fabricflow.com", 20, footerY + 16);
      doc.text("Phone: +94 70 140 6060", 20, footerY + 24);
      doc.text(`Generated on: ${new Date().toLocaleString()}`, 20, footerY + 32);

      // Terms and conditions
      doc.setFontSize(8);
      doc.text("Terms: All orders are subject to availability. Returns accepted within 14 days.", 20, footerY + 45);
